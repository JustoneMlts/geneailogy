    ...
    <Card className="mb-4 sm:mb-6 transition-shadow hover:shadow-lg">
      <CardContent className="p-0">
          ...
          <div className="flex items-start justify-between">
            <div className="flex space-x-2 sm:space-x-3 min-w-0 flex-1">
              <Avatar
                ...
                onClick={() => handleNavigate(getAuthor(post.authorId).id)}
              >
            ...
            <Button variant="ghost" size="icon" className="text-gray-400 hover:text-gray-600 h-6 w-6 sm:h-8 sm:w-8 flex-shrink-0">
        ...
        <div className="px-3 sm:px-4 lg:px-6 pb-2 sm:pb-3 lg:pb-4">
          ...
          {post.image && <img src={post.image} alt="Post image" className="mt-3 sm:mt-4 rounded-lg w-full object-cover max-h-60" />}
          {post.documentUrl && (
            <div className="mt-3 sm:mt-4 w-1/3 rounded-lg border border-gray-200 bg-gray-50 overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2">
                <div className="flex items-center space-x-2 truncate">
                  ...
                  <a href={post.documentUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 over:underline text-sm font-medium truncate">
                    {post.documentName || "Voir le document"}
                  </a>
                </div>
                <Button size="sm" variant="ghost" onClick={() => window.open(post.documentUrl, "_blank")} className="text-gray-600 hover:text-blue-600 flex-shrink-0">Ouvrir</Button>
              </div>
            </div>
          )}
          {post.location && (
        ...
        <div className="px-3 sm:px-4 lg:px-6 py-2 sm:py-3 flex space-x-1">
          <Button variant="ghost" size="sm" onClick={() => handleLike(post)} className={`flex-1 text-xs sm:text-sm ${liked ? "text-blue-600" : "text-gray-600"}`}>
            ...
            <div className="px-3 sm:px-4 lg:px-6 py-3 sm:py-4 space-y-3 sm:space-y-4">
              ...
              <div className="flex space-x-2 sm:space-x-3">
                ...
                <Input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); handleComment(post) } }}
                />
                <Button size="sm" onClick={() => handleComment(post)} disabled={!newComment.trim()}>
